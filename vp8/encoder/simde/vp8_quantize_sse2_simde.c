...
void vp8_regular_quantize_b_sse2_simde(BLOCK *b, BLOCKD *d) {
  char eob = 0;
  ...
  simde__m128i sz0, x0, sz1, x1, y0, y1, x_minus_zbin0, x_minus_zbin1;
  ...
  /* x = abs(z): (z ^ sz) - sz */
  x0 = simde_mm_xor_si128(z0, sz0);
  x1 = simde_mm_xor_si128(z1, sz1);
  x0 = simde_mm_sub_epi16(x0, sz0);
  x1 = simde_mm_sub_epi16(x1, sz1);
  ...
  simde_mm_store_si128((simde__m128i *)(x), x_minus_zbin0);
  simde_mm_store_si128((simde__m128i *)(x + 8), x_minus_zbin1);
  ...
  y0 = simde_mm_sub_epi16(y0, sz0);
  y1 = simde_mm_sub_epi16(y1, sz1);

  simde_mm_store_si128((simde__m128i *)(y), y0);
  simde_mm_store_si128((simde__m128i *)(y + 8), y1);
  ...
  y0 = simde_mm_load_si128((const simde__m128i *)(d->qcoeff));
  y1 = simde_mm_load_si128((const simde__m128i *)(d->qcoeff + 8));

  /* dqcoeff = qcoeff * dequant */
  y0 = simde_mm_mullo_epi16(y0, dequant0);
  y1 = simde_mm_mullo_epi16(y1, dequant1);

  simde_mm_store_si128((simde__m128i *)(d->dqcoeff), y0);
  simde_mm_store_si128((simde__m128i *)(d->dqcoeff + 8), y1);

  *d->eob = eob;
}

void vp8_fast_quantize_b_sse2_simde(BLOCK *b, BLOCKD *d) {
  simde__m128i z0 = simde_mm_load_si128((const simde__m128i *)(b->coeff));
  simde__m128i z1 = simde_mm_load_si128((const simde__m128i *)(b->coeff + 8));
  simde__m128i round0 = simde_mm_load_si128((const simde__m128i *)(b->round));
  simde__m128i round1 = simde_mm_load_si128((const simde__m128i *)(b->round + 8));
  ...
  simde__m128i dequant0 = simde_mm_load_si128((const simde__m128i *)(d->dequant));
  simde__m128i dequant1 = simde_mm_load_si128((const simde__m128i *)(d->dequant + 8));
  simde__m128i inv_zig_zag0 =
      simde_mm_load_si128((const simde__m128i *)(vp8_default_inv_zig_zag));
  simde__m128i inv_zig_zag1 =
      simde_mm_load_si128((const simde__m128i *)(vp8_default_inv_zig_zag + 8));

  simde__m128i sz0, sz1, x0, x1, y0, y1, xdq0, xdq1, zeros, ones;
  ...
  /* x * dequant */
  xdq0 = simde_mm_mullo_epi16(x0, dequant0);
  xdq1 = simde_mm_mullo_epi16(x1, dequant1);

  /* dqcoeff = x * dequant */
  simde_mm_store_si128((simde__m128i *)(d->dqcoeff), xdq0);
  simde_mm_store_si128((simde__m128i *)(d->dqcoeff + 8), xdq1);

  /* build a mask for the zig zag */
  zeros = simde_mm_setzero_si128();

  x0 = simde_mm_cmpeq_epi16(x0, zeros);
  x1 = simde_mm_cmpeq_epi16(x1, zeros);

  ones = simde_mm_cmpeq_epi16(zeros, zeros);

  x0 = simde_mm_xor_si128(x0, ones);
  x1 = simde_mm_xor_si128(x1, ones);

  x0 = simde_mm_and_si128(x0, inv_zig_zag0);
  x1 = simde_mm_and_si128(x1, inv_zig_zag1);

  x0 = simde_mm_max_epi16(x0, x1);

  /* now down to 8 */
  x1 = simde_mm_shuffle_epi32(x0, 0xE);  // 0b00001110

  x0 = simde_mm_max_epi16(x0, x1);

  /* only 4 left */
  x1 = simde_mm_shufflelo_epi16(x0, 0xE);  // 0b00001110

  x0 = simde_mm_max_epi16(x0, x1);

  /* okay, just 2! */
  x1 = simde_mm_shufflelo_epi16(x0, 0x1);  // 0b00000001

  x0 = simde_mm_max_epi16(x0, x1);

  *d->eob = 0xFF & simde_mm_cvtsi128_si32(x0);
}